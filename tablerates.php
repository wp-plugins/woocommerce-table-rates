...
if( !class_exists( 'WPR_Table_Rate' ) ) {
    ...
    class WPR_Table_Rate {
        ...
        private function setup_constants() {
            define( 'WPTR_SHIPPING_VER', '1.4.0' );  // Plugin version
        ...
        public function load_textdomain() {
            ...
            $locale = apply_filters( 'plugin_locale', get_locale(), 'wpr-table-rate' );
            $mofile = sprintf( '%1$s-%2$s.mo', 'wpr-table-rate', $locale );
        ...
        public function settings( $settings ) {
            $new_settings = array(
                array(
                    ...
                    'name'  => '<strong>' . __( 'Plugin Name Settings', 'wpr-table-rate' ) . '</strong>',
                    'desc'  => __( 'Configure Plugin Name Settings', 'wpr-table-rate' ),
                    'type'  => 'header',
                )
            );
